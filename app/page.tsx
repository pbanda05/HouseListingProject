 cat << 'EOF' > fix.js
const fs = require('fs');
let code = `import Link from 'next/link';
    ...
    @@div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-indigo-600 selection:text-white">
      ...
      @@nav className="w-full border-b border-slate-200 bg-white">
        @@div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          @@/div>
          @@div className="text-sm font-medium text-slate-500 hover:text-slate-900 cursor-pointer transition">
            Sign In
          @@/div>
        @@/div>
      @@/nav>
...
console.log('\n✅ Bypassed the glitch! app/page.tsx has been successfully written.');
EOF
node fix.js
rm fix.js